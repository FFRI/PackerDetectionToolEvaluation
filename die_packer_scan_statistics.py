...
def is_packer_type(type_: str) -> bool:
    return type_ == "protector" or type_ == "packer" or type_ == "installer"


def filter_only_packed_detection_reasons(detects: List[dict]) -> List[dict]:
    return [detect for detect in detects if is_packer_type(detect["type"])]


def packed_detection_reasons_count(detects: List[dict]) -> int:
    return len(filter_only_packed_detection_reasons(detects))
    ...
    for name in os.listdir(path):
        ...
        for i in json_data:
            if "scan" not in i or "detects" not in i["scan"] or "detectable" not in i:
                print(f"Scan failed samples", i["path"], file=sys.stderr)
                print("This sample will be skipped", file=sys.stderr)
                n_samples_failed += 1
                continue
            ...
            if i[
                "detectable"
            ]:  # Contains true label. Some samples are excessively detected as packed
                if packed_detection_reasons_count(i["scan"]["detects"]) == 1:
                    purely_detected_as_packed += 1
                else:
                    excessively_detected_as_packed += 1
            else:  # Does not contain true label. Some samples are excessively detected as packed
                if packed_detection_reasons_count(i["scan"]["detects"]) == 0:
